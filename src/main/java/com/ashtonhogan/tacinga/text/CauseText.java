...
public class CauseText implements Text {

    private final Scalar<String> origin;

    public CauseText(final String input) {
        this(input, StandardCharsets.UTF_8);
    }

    public CauseText(final String input, final Charset cset) {
        this(() -> new String(input.getBytes(cset), cset));
    }

    private CauseText(final Scalar<String> scalar) {
        this.origin = scalar;
    }
